...
function createPointGeometry(point) {
  ...
  const geometry = parseGeometryData(point.geometry) || point;
  if (geometry?.declaredClass === "esri.geometry.Point") {
    return geometry;
  }

  const hasExplicitLonLat = firstFiniteNumber(point?.longitude, point?.lon) !== null
    && firstFiniteNumber(point?.latitude, point?.lat) !== null;
  ...
  const geometryData = {
    ...
    spatialReference: hasExplicitLonLat
      ? { wkid: 4326 }
      : geometry?.spatialReference || point?.spatialReference || { wkid: 4326 },
  };
...
function applyResolvedElevation(point, elevacion, geometry = null, metadata = {}) {
  ...
  const sourceGeometry = parseGeometryData(geometry || point?.geometry) || point;
  const hasExplicitLonLat = firstFiniteNumber(point?.longitude, point?.lon) !== null
    && firstFiniteNumber(point?.latitude, point?.lat) !== null;
  const x = firstFiniteNumber(point?.longitude, point?.lon, point?.x, sourceGeometry?.longitude, sourceGeometry?.lon, sourceGeometry?.x);
  const y = firstFiniteNumber(point?.latitude, point?.lat, point?.y, sourceGeometry?.latitude, sourceGeometry?.lat, sourceGeometry?.y);
  ...
  return {
    ...
    geometry: Number.isFinite(x) && Number.isFinite(y)
      ? {
          ...
          spatialReference: hasExplicitLonLat
            ? { wkid: 4326 }
            : sourceGeometry?.spatialReference || point?.spatialReference || { wkid: 4326 },
        }
      : point?.geometry || null,
  };
}
