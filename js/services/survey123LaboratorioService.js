...
export async function enqueueLaboratorioSurvey123Submission(assay, collar) {
  ...
  if (String(assay.muestra_id || "").trim().toUpperCase() === "NO HAY") {
    return { skipped: true, reason: "Muestra ID 'NO HAY': respuesta Survey123 no enviada." };
  }
...
export async function syncLaboratorioSurvey123Submissions(actionMode = "all") {
  ...
  for (const item of items) {
    try {
      if (String(item.payload?.muestra_id || "").trim().toUpperCase() === "NO HAY") {
        skipped += 1;
        continue;
      }
