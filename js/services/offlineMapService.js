...
const DEBUG_OFFLINE_MAP = false;

function debugOfflineMap(...args) {
  if (DEBUG_OFFLINE_MAP) {
    console.log(...args);
  }
}
...
export async function downloadVisibleArea(view, options = {}) {
  debugOfflineMap("[offline-map] downloading visible area");
  ...
  debugOfflineMap("[offline-map] calculated tiles:", tiles.length);
  ...
  for (const tile of tiles) {
    ...
    if (existingTile && isValidStoredTile(existingTile)) {
      debugOfflineMap("[offline-map] tile already exists", tile.z, tile.x, tile.y);
      skipped++;
    ...
    try {
      debugOfflineMap("[offline-map] attempting tile fetch", {
        origin: location.origin,
      ...
      debugOfflineMap("[offline-map] tile response", {
        ok: response.ok,
      ...
      debugOfflineMap("[offline-map] downloaded tile", tile.z, tile.x, tile.y);
      await saveTile({
      ...
      debugOfflineMap("[offline-map] saved tile", tile.z, tile.x, tile.y);
      downloaded++;
    } catch (error) {
      errors++;
...
async function renderTileLayer() {
  ...
  debugOfflineMap("[offline-map] loading tiles from IndexedDB");
  ...
  for (let x = range.minX; x <= range.maxX; x++) {
    for (let y = range.minY; y <= range.maxY; y++) {
      ...
      if (!isValidStoredTile(tile)) {
        debugOfflineMap("[offline-map] missing tile", offlineMapState.zoom, x, y);
        continue;
      }
...
export async function loadOfflineMap(container, options = {}) {
  debugOfflineMap("[offline-map] offline mode");
  if (!container) {
  ...
  if (!offlineMapState.hasTiles) {
    debugOfflineMap("[offline-map] no offline tiles found");
    debugOfflineMap("[offline-map] using grid fallback");
  }
