...
export function renderLaboratorioForm({ activeProject, activeProjectAssays, activeProjectLaboratorios = [], editingLaboratorio }) {
  ...
  return `
    <form id="laboratorio-form" class="stack" autocomplete="off">
      ...
      <div class="grid-fields">
        <label class="field"><span>Contraccion</span><input class="input" name="contraccion" type="number" inputmode="decimal" step="any" value="${escapeHtml(String(contraccionValue))}"></label>
        <label class="field"><span>Absorcion</span><input class="input" name="absorcion" type="number" inputmode="decimal" step="any" value="${escapeHtml(String(absorcionValue))}"></label>
      </div>
      <label class="field">
