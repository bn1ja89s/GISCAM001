import { escapeHtml, formatDateTime, formatDecimalInput, generarIdDesdeTecnicoYCollar, toNullableNumber } from "../core/helpers.js";
...
function getAssayOrderValue(assay) {
  const dateValue = Date.parse(assay.fecha_creacion || assay.fecha_modificacion || "");
  if (Number.isFinite(dateValue)) {
    return dateValue;
  }

  return Number(assay.id_local || 0);
}

function getNextDesdeForCollar(collar, assays) {
  const collarAssays = assays
    .filter((assay) => assay.collar_uuid === collar?.uuid && toNullableNumber(assay.hasta) != null)
    .sort((left, right) => getAssayOrderValue(left) - getAssayOrderValue(right));
  const previousAssay = collarAssays.at(-1);

  return previousAssay ? previousAssay.hasta : 0;
}
...
export function renderAssayForm({ activeProject, activeProjectCollars, activeProjectAssays = [], editingAssay, captureFlow, selectedAssayCollarUuid = "" }) {
  ...
  const nextDesde = getNextDesdeForCollar(selectedCollar, activeProjectAssays);
  const selectedMuestraId = assay.muestra_id || nextMuestraId;
  const desdeValue = editingAssay ? assay.desde : nextDesde;
  ...
  const descripcionOptions = [
    "Horizonte de interés seleccionado para muestreo de prueba; intervalo representativo del material objetivo.",
    "Horizonte superficial de arcilla oscura, húmeda, con contenido arcilloso dominante y posible materia orgánica. Horizonte de relleno para completar la secuencia estratigráfica; no seleccionado para muestreo.",
    "Material arcilloso residual producto de alteración del saprolito; horizonte no principal. Horizonte de relleno para completar la secuencia estratigráfica; no seleccionado para muestreo.",
    "Primer subintervalo del horizonte de interés seleccionado para muestreo de prueba.",
    "Roca base granodiorítica alterada, con fracturación moderada y transición a saprolito. Horizonte de relleno para completar la secuencia estratigráfica; no seleccionado para muestreo.",
    "Saprolito derivado de granodiorita, con matriz arcillosa y alteración caolinítica incipiente. Horizonte de relleno para completar la secuencia estratigráfica; no seleccionado para muestreo.",
    "Segundo subintervalo del horizonte de interés seleccionado para muestreo de prueba.",
    "Transición saprolítica con matriz arcillosa, coloración ocre y presencia variable de caolinización. Horizonte de relleno para completar la secuencia estratigráfica; no seleccionado para muestreo.",
  ];
  ...
  const contaminanteOptionsForRender = assay.contaminantes && !contaminanteOptions.includes(assay.contaminantes)
    ...
    : contaminanteOptions;
  const currentDescripcion = assay.descripcion || "";
  const descripcionOptionsForRender = currentDescripcion && !descripcionOptions.includes(currentDescripcion)
    ? [currentDescripcion, ...descripcionOptions]
    : descripcionOptions;
  ...
  return `
    <form id="assay-form" class="stack" autocomplete="off">
        ...
        <label class="field">
          ...
          <select class="select" name="collar_uuid" required>
            ${activeProjectCollars
              .map(
                (collar) => `<option value="${escapeHtml(collar.uuid)}" data-next-muestra-id="${escapeHtml(getNextMuestraId(activeProject, collar, activeProjectAssays))}" data-next-desde="${escapeHtml(formatDecimalInput(getNextDesdeForCollar(collar, activeProjectAssays)))}" ${collar.uuid === selectedCollarUuid ? "selected" : ""}>${escapeHtml(collar.hole_id)} - ${escapeHtml(collar.tipo || "-")}</option>`,
              )
              .join("")}
          </select>
        </label>
      ...
      <div class="grid-fields grid-fields--3">
        ${renderNumberStepper({ name: "desde", label: "Desde", value: desdeValue, min: "0", required: true })}
      ...
      <label class="field">
        ...
        <select class="select" name="descripcion">
          <option value="" ${currentDescripcion === "" ? "selected" : ""}>Selecciona</option>
          ${renderOptions(descripcionOptionsForRender, currentDescripcion)}
        </select>
      </label>
      <div class="grid-fields grid-fields--3">
        <label class="field">
