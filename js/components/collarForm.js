import { escapeHtml, formatDateTime, formatDecimalInput, todayValue } from "../core/helpers.js";
...
function renderCoordinateRow(collar) {
  return `
    <div class="capture-coordinate-strip" aria-label="Resumen de coordenadas capturadas">
      <span class="capture-coordinate-pill"><strong>Lat</strong>${escapeHtml(formatDecimalInput(collar.latitude) || "-")}</span>
      <span class="capture-coordinate-pill"><strong>Lon</strong>${escapeHtml(formatDecimalInput(collar.longitude) || "-")}</span>
      <span class="capture-coordinate-pill"><strong>${renderIcon("mountain")} Z</strong>${escapeHtml(formatDecimalInput(collar.elevacion) || "-")} m</span>
    </div>
  `;
}
...
function renderPrimaryFieldSet(collar, currentTipo) {
  ...
  const currentLocationRaw = collar.localizacion || "";
  const currentLocation = ["sector", "plataforma"].includes(currentLocationRaw.toLowerCase())
    ? currentLocationRaw.toUpperCase()
    : currentLocationRaw;
  ...
  return `
    <div class="grid-fields">
      <label class="field">
        ...
        <input class="input" name="prof_total" type="text" inputmode="decimal" value="${escapeHtml(formatDecimalInput(collar.prof_total))}">
      </label>
      <label class="field">
        <span>Fecha</span>
  ...
  return `
    <div class="panel-tile stack ${compact ? "capture-sheet__readonly" : ""}">
      ...
      <div class="grid-fields grid-fields--3">
        <label class="field">
          <span>Latitud</span>
          <input class="input" name="latitude" ${coordinateAttrs} value="${escapeHtml(formatDecimalInput(collar.latitude))}">
        </label>
        <label class="field">
          <span>Longitud</span>
          <input class="input" name="longitude" ${longitudeAttrs} value="${escapeHtml(formatDecimalInput(collar.longitude))}">
        </label>
        <label class="field">
          <span>Elevacion</span>
          <input class="input" name="elevacion" readonly value="${escapeHtml(formatDecimalInput(collar.elevacion))}">
        </label>
      </div>
      <div class="grid-fields">
        <label class="field">
          <span>Este</span>
          <input class="input" name="este" readonly value="${escapeHtml(formatDecimalInput(collar.este))}">
        </label>
        <label class="field">
          <span>Norte</span>
          <input class="input" name="norte" readonly value="${escapeHtml(formatDecimalInput(collar.norte))}">
        </label>
      </div>
  ...
  return `
    <form id="collar-form" class="stack collar-form ${isSheet ? "collar-form--sheet" : ""}" autocomplete="off">
      ...
      ` : `
        <div class="collar-summary panel-tile stack">
          ...
          <div class="list-item__meta">
            <span>Lat ${escapeHtml(formatDecimalInput(collar.latitude) || "-")}</span>
            <span>Lon ${escapeHtml(formatDecimalInput(collar.longitude) || "-")}</span>
            <span>Elev ${escapeHtml(formatDecimalInput(collar.elevacion) || "-")}</span>
      ...
      <div class="inline-row ${isSheet ? "capture-sheet__actions" : ""}">
        ...
        ${editingCollar ? `<button class="ghost-button" type="button" data-action="focus-collar" data-uuid="${escapeHtml(editingCollar.uuid)}">Enfocar en mapa</button>` : ""}
