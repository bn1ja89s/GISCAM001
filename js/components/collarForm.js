  ...
  return `
    <div class="grid-fields">
      <label class="field">
        ...
        <input class="input" name="prof_total" type="number" inputmode="decimal" step="any" value="${escapeHtml(String(collar.prof_total ?? ""))}">
      </label>
      <label class="field">
        <span>Fecha</span>
...
function renderCoordinatesPanel(collar, activeProject, { compact = false, editable = false } = {}) {
  const coordinateAttrs = editable
    ? 'type="number" inputmode="decimal" step="any" min="-90" max="90" required'
    : "readonly";
  const longitudeAttrs = editable
    ? 'type="number" inputmode="decimal" step="any" min="-180" max="180" required'
    : "readonly";
