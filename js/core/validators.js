...
export function validateCollar(data) {
  ...
  } else if (!Number.isFinite(Number(data.latitude)) || Number(data.latitude) < -90 || Number(data.latitude) > 90) {
    errors.push("La latitud debe ser un numero entre -90 y 90.");
  }

  if (data.longitude === "" || data.longitude === null || data.longitude === undefined) {
    errors.push("Debes capturar el punto del collar en el mapa.");
  } else if (!Number.isFinite(Number(data.longitude)) || Number(data.longitude) < -180 || Number(data.longitude) > 180) {
...
export function validateSurvey(data) {
  ...
  if (!isFiniteNumber(data.dip) || Number(data.dip) < -90 || Number(data.dip) > 90) {
